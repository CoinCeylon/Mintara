...
import { Transaction, deserializeAddress, ForgeScript } from "@meshsdk/core"
...
export default function MarketplacePage() {
  ...
  const handleMintNFT = async () => {
    ...
    try {
      ...
      // Prepare asset details - clean the name for Cardano compatibility
      ...
      // Find the correct policy ID
      const policyIds = Object.keys(metadata721)
      const matchingPolicyId = policyIds.find((id) => Object.keys(metadata721[id] ?? {}).includes(selectedReward.name))
      ...
      // Create shortened metadata to avoid length limits
      const imageRef = `reward_${selectedReward.id.slice(0, 8)}`
        ...
        "721": {
          [matchingPolicyId]: {
            [assetName]: {
              ...
              description: (sourceMetadata.description || "NFT Reward").slice(0, 32),
              image: imageRef,
      ...
      // Create a ForgeScript for native token minting
      const forgeScript = ForgeScript.withOneSignature(changeAddress)

      // Build transaction using the lower-level approach
      const tx = new Transaction({ initiator: wallet })

      // Add the mint using the forge script approach
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes("UTxO")) {
          ...
          errorMessage = "Collateral issues. Please ensure you have sufficient ADA."
        ...
        } else if (errorMessage.includes("CBOR") || errorMessage.includes("buffer")) {
          ...
          <CardTitle className="text-green-400 flex flex-col gap-2 md:flex-row items-center justify-between">
            ...
            {selectedReward && (
              <div className="flex justify-center">
                <Button variant={"default"} onClick={handleMintNFT} disabled={isMinting}>
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 ${getRarityColor(
                        ...
                        isSelected ? "ring-1 ring-green-400 ring-offset-2 ring-offset-background" : ""
                      }`}
                    >
                      {isSelected && (
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <Image
                          unoptimized
                          src={item.rewardCatalog.imageUrl || "/placeholder.svg"}
                          alt={item.name}
                          width={200}
                          height={200}
                      ...
                      <h3 className="font-semibold text-green-400 mb-1">{item.name}</h3>
                      <div className="flex gap-2 mb-2">
                        <Badge className={getRarityColor(item.rewardCatalog.rarity)}>{item.rewardCatalog.rarity}</Badge>
                      </div>
                      <p className="text-sm text-green-600 mb-2">{item.rewardCatalog.type}</p>
                      <h3 className="text-sm text-green-400 mb-1">{item.rewardCatalog.description}</h3>
                    </div>
                  )
                })}
            </div>
